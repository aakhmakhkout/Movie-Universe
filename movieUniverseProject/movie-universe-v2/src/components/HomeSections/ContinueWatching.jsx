      ...
      <div className="flex gap-10">
        {ContinueWatchingData.map((items) => {
          return (
            <div
              key={items.id}
              className="w-50 h-90 flex flex-col gap-2 cursor-pointer group"
            >
              <div className="w-full h-75 relative bg-black/20 overflow-hidden rounded-xl group-hover:border">
                ...
                <div className="absolute top-0 w-full h-full bg-black/20">
                  <div className="w-full h-full flex justify-center items-center">
                    <button className="border-2 border-white p-2 rounded-full playBtn group-hover:bg-[#22b365] group-hover:border-0 cursor-pointer">
                      <Play size={36} strokeWidth={1.5} />
                    </button>
                  </div>
                </div>
                <button className="absolute top-2 right-2 hidden group-hover:flex cursor-pointer bg-[#21213a] rounded-full p-1">
                  <X size={20} strokeWidth={1.5} />
                </button>
              </div>
              <div className="flex justify-center items-center">
                <h1 className="text-center px-2 text-white/80 group-hover:text-white">
                  {items.title}
                </h1>
              </div>
            </div>
